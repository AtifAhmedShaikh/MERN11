...
export default function Cart() {
  ...
  const handleQuantity = (productId, newQuantity) => {
    const copyCart = { ...cart };
    copyCart[productId].qty = newQuantity;
    setCart({ ...copyCart });
  };
  const testKey=()=>{
    const key1=import.meta.env.VITE_SECRET_KEY;
    const key2=import.meta.env.VITE_PUBLISH_KEY
        ...
        {Object.keys(cart).map((KeyItem, index) => {
          return (
            <div key={index} className="">
              ...
              <div>
                <span
                  onClick={() => handleQuantity(KeyItem, cart[KeyItem].qty + 1)}
                >
                  {" "}
                  ➕
                </span>
                ...
                <span
                  className="cursor-pointer px-4 bg-red-300"
